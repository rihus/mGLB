...
for correction, image_fname in CORR_DICT.items():
    ...
    mean_glb= create_new_folder(PARENT_DIR, folder_name=f"{CORR_TYPE}_mean_glb_thresholds")
    all_dirs = []
    ...
    with open(os.path.join(mean_glb, "all_dirs_list.txt"), 'w', encoding="utf-8") as f:
        f.write('\n'.join(all_dirs))
    ...
    for folder1 in all_dirs:
        ...
        img_path = os.path.join(folder1, OPTION) if OPTION in filelist2 else None
        msk_path = os.path.join(folder1, 'img_ventilation_mask.nii.gz'
                                ) if 'img_ventilation_mask.nii.gz' in filelist2 else None

        if not img_path or not msk_path:
            print("MR image or Mask not found")
            continue

        print(f"Current folder is: {folder1}")
        imgs_data = nib.load(img_path).get_fdata()
        msks_data = nib.load(msk_path).get_fdata()
        N_SUBJ += 1
        # Apply mask
        masked_data = imgs_data * msks_data
        oneD_masked_nZero = masked_data[masked_data != 0]
        # Plot original histogram
        plot_histogram(oneD_masked_nZero, subject_name, correc=CORR_TYPE,
                    save_path=mean_glb, suffix="full")
        # Normalize and clip at 99th percentile
        single_mean_value = np.mean(oneD_masked_nZero)
        single_mean_norm = oneD_masked_nZero / single_mean_value
        mean_percentile = np.percentile(single_mean_norm, 99)
        single_mean_norm = np.clip(single_mean_norm, None, mean_percentile)
        # Plot normalized histogram
        plot_histogram(single_mean_norm, subject_name, correc=CORR_TYPE,
                    save_path=mean_glb, suffix="mean_norm")
        # Append to global array
        big1d_mean_healthy.extend(single_mean_norm)
    print("Total number of Healthy Subjects included:", N_SUBJ)

    ## Plot the histogram for all healthy Data, BoxCox Tranformation
    plot_histogram(big1d_mean_healthy, "healthy",  correc=CORR_TYPE,
                   save_path=mean_glb, suffix="mean")
    # ##Plot and save big data file and
    data_mean = os.path.join(mean_glb, f"{CORR_TYPE}_mean_big1D")
    np.save(data_mean, big1d_mean_healthy)
    # ##Box Cox transformation and transformed space histogram, and save healthy data bins
    zScores_mean = boxcox_zscores_histogram(big1d_mean_healthy, mean_glb,
                                      CORR_TYPE, "glb_mean")
    # ##Histogram overlay and save the image
    hist_overlay(big1d_mean_healthy, zScores_mean, mean_glb,
                 CORR_TYPE, "glb_mean")
